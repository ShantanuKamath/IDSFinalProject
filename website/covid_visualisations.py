...
def covid_visualisations():
    ...
    st.markdown("""Before we begin studying the effect that covid had on the stock market, we perform a preliminary analysis to understand the progression of covid cases in the US right from when the initial cases started getting reported in early January, 2020 till date.""")
...
We identify these as our time durations of interest and use them subsequently to understand how the stock market performed during these two peaks""")
    ...
    st.text("")
    st.subheader("Which were the most notorious states when it comes to covid?")
    st.markdown("""Once we had seen a high level view of the covid trend in the US, we wanted to how covid affected each state. We wanted to identify which states were the worst affected by covid, and which states were the least affected.
For this we plotted a bar chart to visualise the total covid cases per state.""")
    ...
    st.text("")
    st.subheader("Did all states peak at the same time?")
    st.markdown("""Once we had a high level view of how each US state was affected by Covid, we decided to dig a little deeper and scrutanise the covid trends at a state level.
    ...
    dropdown_state = st.selectbox('Enter a primary state',states, index=0)
    dropdown_comp_state = st.selectbox('Enter a secondary state for comparison',states, index=1)
